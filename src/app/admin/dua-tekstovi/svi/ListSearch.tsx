...
export default function DuaListSearch({ tekstovi }: { tekstovi: DuaText[] }) {
      ...
      {filtered.length === 0 ? (
        <div className="bg-white rounded-2xl p-12 text-center border border-dashed border-zinc-200">
          <p className="text-zinc-400">
            {query.trim() ? `Nema rezultata za "${query}"` : 'Nema tekstova dova.'}
          </p>
        </div>
      ) : (
            ...
            <div key={t.id}
              ...
              <div className="flex items-center gap-1 flex-shrink-0">
                <Link href={`/admin/dua-tekstovi/${t.id}/edit`} title="Uredi"
                  className="p-1.5 transition-colors hover:opacity-80" style={{ color: '#1D4ED8' }}>
                  <Pencil size={13} />
                </Link>
                <Link href={`/zikrovi/tekstovi/${t.id}`} target="_blank" title="Pregledaj"
