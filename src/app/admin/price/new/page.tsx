...
export default function NewPricaPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/price', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ title, content, author, imageUrl: finalImageUrl, published }),
      })
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
        ...
        <h1 className="text-2xl font-black text-zinc-900 mb-6">Dodaj poučnu priču</h1>
        ...
        <div className="bg-white rounded-2xl p-6 border border-zinc-100 shadow-sm space-y-4">
          <div>
            ...
            <input type="text" value={title} onChange={e => setTitle(e.target.value)}
              placeholder="npr. Kada je znanje postalo iman"
          ...
          <div>
            <label className="block text-sm font-semibold text-zinc-700 mb-1.5">
              Autor / Izvor <span className="text-zinc-400 font-normal">(opciono)</span>
            </label>
            <input type="text" value={author} onChange={e => setAuthor(e.target.value)}
              placeholder="npr. Ibn al-Mubārak"
              className="w-full px-4 py-2.5 border border-zinc-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-brand text-sm" />
          </div>
          <div>
            <label className="block text-sm font-semibold text-zinc-700 mb-1.5">
          ...
          <div>
            <label className="block text-sm font-semibold text-zinc-700 mb-1.5">Sadržaj priče *</label>
            <textarea value={content} onChange={e => setContent(e.target.value)}
              placeholder="Unesite tekst priče..."
              rows={16}
