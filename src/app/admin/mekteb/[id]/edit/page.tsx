...
export default function EditMektebPage() {
  ...
  useEffect(() => {
    fetch(`/api/mekteb/${id}`).then(r => r.json()).then(d => {
      setTitle(d.title || ''); setContent(d.content || ''); setAuthor(d.author || '')
      setImageUrl(d.imageUrl || ''); setCategory(d.category || 'opste'); setPublished(d.published ?? true)
      setLoading(false)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/mekteb/${id}`, {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
        ...
        <h1 className="text-2xl font-black text-zinc-900 mb-6">Uredi Mekteb sadržaj</h1>
        {loading ? (
