...
import { ArrowLeft, Loader2, Upload, X, ImagePlus, FileText, Video } from 'lucide-react'

type Mode = null | 'tekst' | 'video'

export default function NewZikrPage() {
  ...
  const handleAudioUpload = async (file: File) => {
    ...
    } catch { setError('Greška pri uploadu audio fajla') }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/zikr-tekstovi', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ title, content, author, audioUrl: finalAudioUrl, thumbnailUrl: finalThumbnailUrl, published }),
      })
      const data = await res.json()
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
        ...
        <h1 className="text-2xl font-black text-zinc-900 mb-2">Dodaj zikr</h1>
        <p className="text-zinc-400 text-sm mb-8">Odaberi tip sadržaja koji dodaješ</p>
        ...
        {mode === null && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <button
              ...
              className="group flex flex-col items-start gap-3 p-6 bg-white border-2 border-zinc-200 rounded-2xl hover:border-brand hover:shadow-md transition-all text-left">
              ...
              <div>
                <p className="font-bold text-zinc-900 text-base">Tekst i Audio</p>
                <p className="text-zinc-500 text-sm mt-1">Dodaj tekst zikra sa opcionalnim audio snimkom i naslovnom slikom</p>
              </div>
            </button>
            <button
              onClick={() => router.push('/admin/videos/new?category=zikrovi')}
              className="group flex flex-col items-start gap-3 p-6 bg-white border-2 border-zinc-200 rounded-2xl hover:border-brand hover:shadow-md transition-all text-left">
              ...
              <div>
                <p className="font-bold text-zinc-900 text-base">Video</p>
                <p className="text-zinc-500 text-sm mt-1">Dodaj YouTube/Vimeo video zikra sa naslovom i tekstom zikra u opisu</p>
              </div>
            </button>
          </div>
        )}
        ...
        {mode === 'tekst' && (
          <>
            <div className="flex items-center gap-2 mb-6">
              <button onClick={() => setMode(null)} className="text-zinc-400 hover:text-zinc-700 transition-colors">
                <ArrowLeft size={16} />
              </button>
              <span className="text-sm text-zinc-500">Tekst i Audio</span>
            </div>

            <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm space-y-5">
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1.5">Naziv zikra *</label>
                <input type="text" value={title} onChange={e => setTitle(e.target.value)}
                  placeholder="Npr. Subhanallah — zikr jutra i večeri"
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input type="url" value={thumbnailUrl}
                    onChange={e => { setThumbnailUrl(e.target.value); setThumbFile(null); setThumbPreview(null) }}
                    placeholder="https://..."
                  ...
                  <button type="button" onClick={() => thumbInputRef.current?.click()}
                    className="flex items-center gap-1.5 px-4 py-2.5 border border-gray-200 rounded-xl text-sm text-zinc-600 hover:bg-zinc-50 transition-colors whitespace-nowrap">
                ...
                {(thumbPreview || thumbnailUrl) && (
                  <div className="mt-2 relative inline-block">
                    ...
                    <img src={thumbPreview || thumbnailUrl} alt="Preview" className="rounded-lg max-h-40 object-cover" />
                    <button type="button"
                      onClick={() => { setThumbFile(null); setThumbPreview(null); setThumbnailUrl('') }}
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1.5">
                  Tekst zikra <span className="text-gray-400 font-normal">(opciono)</span>
                </label>
                <textarea value={content} onChange={e => setContent(e.target.value)}
                  placeholder="Unesite tekst zikra, dove ili suplikacije..."
                  rows={10}
