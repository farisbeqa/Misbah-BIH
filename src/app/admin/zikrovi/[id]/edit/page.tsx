...
export default function EditZikrTextPage() {
  ...
  useEffect(() => {
    fetch(`/api/zikr-tekstovi/${id}`).then(r => r.json()).then(d => {
      setTitle(d.title || ''); setContent(d.content || ''); setAuthor(d.author || '')
      setAudioUrl(d.audioUrl || ''); setAudioUrlInput(d.audioUrl || '')
      setThumbnailUrl(d.thumbnailUrl || ''); setPublished(d.published ?? true)
      setLoading(false)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/zikr-tekstovi/${id}`, {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
        ...
        <h1 className="text-2xl font-black text-gray-900 mb-6">Uredi zikr (Tekst i Audio)</h1>
