...
export async function generateMetadata({ params }: { params: { id: string } }) {
  try {
    const t = await prisma.duaText.findUnique({ where: { id: parseInt(params.id) } })
    return t ? { title: `${t.title} — Dove`, alternates: { canonical: `/zikrovi/tekstovi/${params.id}` } } : { title: 'Misbah EDU' }
  } catch { return { title: 'Misbah EDU' } }
}

export default async function DuaTextPage({ params }: { params: { id: string } }) {
  const id = parseInt(params.id)
  if (isNaN(id)) notFound()

  const tekst = await prisma.duaText.findUnique({ where: { id } }).catch(() => null)
  if (!tekst || !tekst.published) notFound()

  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-10 sm:py-16" style={{ minHeight: '60vh' }}>
      <Link href="/zikrovi/tekstovi"
        className="inline-flex items-center gap-2 text-sm mb-8 hover:text-brand transition-colors"
        style={{ color: '#978A81' }}>
        <ArrowLeft size={14} /> Sve dove
      </Link>
      ...
      <header className="mb-6">
        ...
        {tekst.author && (
          <p className="text-sm font-medium" style={{ color: '#1D4ED8' }}>{tekst.author}</p>
        )}
      </header>
